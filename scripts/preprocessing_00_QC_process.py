...
#     # --- Level 2: Filter by RT & event
#     trials_df = trials_df.loc[trials_df['eid'].isin(valid_sessions['eid'].unique())]
#     print(trials_df['eid'].nunique()) #TODO:
#     trials_df["response_times_from_stim"] = trials_df["response_times"] - trials_df["stimOn_times"]
...
#     #TODO:这里去除4个pids? 还要对应处理table保证不包含这4个pids; 哪里
#     full_release_df = full_release_df[~full_release_df['pid'].isin(C.PIDS_WITHOUT_ILBLSORTOR)]
#     print(f"Before: {len(full_release_df)}, After: {full_release_df.shape[0]}")
...
#     qc_table_wide.to_csv(C.DATAPATH / 'QC_summary_table_wide.csv', index=False)
