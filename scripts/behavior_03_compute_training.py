...
def merge_full_training_trials(data=None, save_results=True, datapath='../data' ):
...
def compute_training_history(data_info_unique,):
    list_perf_easy = []
    not_included_subjs = []
    # failed_subjs = []
    ...
    training_table['age_group'] = training_table['mouse_age'].apply(lambda age: 'old' if age > 300 else 'young')
...
if __name__ == "__main__":
    ...
    trials_table_file = os.path.join(config.datapath, 'ibl_included_eids_trials_table2025_full.csv')
    try:
        trials_table = pd.read_csv(trials_table_file) #236118 trials
    except Exception as err:
        print(f'errored: {err}')
    ...
    merged_training_trials_file = 'subjectTraining_Trials_table_149subjs_2025.parquet'
    if os.path.exists(os.path.join(config.datapath, merged_training_trials_file)):
        all_subj_tt_df = pd.read_parquet(os.path.join(config.datapath, merged_training_trials_file), engine='pyarrow')
    else:
        all_subj_tt_df = merge_full_training_trials(data=data_info_unique, datapath=datapath, save_results=True)
        all_subj_tt_df.to_parquet(os.path.join(config.datapath, merged_training_trials_file), engine='pyarrow', compression='snappy')
    ...
    if save_results:
        ...
        training_table.to_parquet(os.path.join(config.datapath,'training_history_149subjs_2025_NEW_v2.parquet'), engine='pyarrow', compression='snappy')
