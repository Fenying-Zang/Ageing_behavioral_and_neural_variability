...
def permut_test(data, metric, labels1, labels2, n_permut=10000, shuffling='labels1', plot=False, return_details=False, mark_p=None, n_cores=1, title=None, one_sided=True):
    """
    ...
    if n_cores == 1:
        for i in trange(n_permut, desc="Running permutations"):
    ...
    if one_sided:
        ...
        p = np.sum(np.abs(null_dist) >= np.abs(observed_val)) / n_permut
