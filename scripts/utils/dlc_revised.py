...
revised by Fenying, paras, outputs
...
def compute_motion_energy_hist(camera_dict, trials_df, eid): #TODO:eid  [],
    """
    ...
    for cam in camera_dict.keys():
        if (camera_dict[cam]['motion_energy'] is not None and len(camera_dict[cam]['motion_energy']) > 0
            ...
            try:
                ...
                motion_energy = camera_dict[cam]['motion_energy']
                try:
                    ...
                    me_all_p = [motion_energy[start_idx[i]:end_idx[i]] for i in range(len(start_idx))]
                    me_all = [m for m in me_all_p if len(m) > 0]
                    ...
                    curr['times'] = times
                    curr['me_mean'] = me_mean
                    curr['me_std'] = me_std
                    df_curr = pd.DataFrame.from_dict(curr) 
                    df_curr['camera'] = cam
                    df_curr['eid'] = eid
                    list_df_curr.append(df_curr)
                except ValueError:
                    print(f"{cam}Camera camera.times are outside of the trial windows")
                    missing_data.append(cam)
            except AttributeError:
                print(f"Cannot load motion energy and/or times data for {cam} camera")
                missing_data.append(cam)
        else:
            print(f"Data missing or empty for motion energy and/or times data for {cam} camera")
            missing_data.append(cam)
    results_df = pd.concat(list_df_curr, ignore_index=True)
    return results_df
...
def compute_speed_hist(dlc_df, cam_times, trials_df, feature='paw_r', cam='left'):
    """
    ...
    ave_speed_across_trials = pd.DataFrame.from_dict(dict(zip(trials_df[f'speed_{feature}'].index, trials_df[f'speed_{feature}'].values))).mean(axis=1)
    curr['times'] = times
    curr[f'ave_speed_{feature}'] = ave_speed_across_trials
